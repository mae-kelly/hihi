...
const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({ metrics }) => {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8 glass-panel rounded-xl p-6 border-red-500/50 bg-red-500/10">
        <div className="flex items-center gap-4">
          <AlertCircle className="w-8 h-8 text-red-400 animate-pulse" />
          <div>
            <h3 className="text-2xl font-bold text-red-400">CRITICAL VISIBILITY FAILURE</h3>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className="glass-panel rounded-2xl p-6">
          ...
          <div className="mt-4 text-xs text-red-400">100% in CMDB</div>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-6 border-red-500/30">
          ...
          <div className="mt-4 text-xs text-red-400">↓ 60.83% below target</div>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-6">
          ...
          <div className="text-3xl font-bold text-yellow-400">{metrics.splunkCoverage}%</div>
          <div className="text-sm text-gray-400">Splunk Coverage</div>
          <div className="mt-4 text-xs text-yellow-400">Partial deployment</div>
        </div>

        <div className="glass-panel rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <Cloud className="w-6 h-6 text-green-400" />
            <span className="text-xs bg-green-500/20 text-green-400 px-2 py-1 rounded">GOOD</span>
          </div>
          <div className="text-3xl font-bold text-green-400">{metrics.chronicleCoverage}%</div>
          <div className="text-sm text-gray-400">Chronicle Coverage</div>
          <div className="mt-4 text-xs text-green-400">Near complete</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div className="glass-panel rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-red-400 mb-4">TOP CRITICAL GAPS</h3>
        ...
        <div className="glass-panel rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-yellow-400 mb-4">COMPLIANCE STATUS</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
      ...
      <div className="glass-panel rounded-2xl p-6 mb-8">
        <h3 className="text-lg font-semibold text-cyan-300 mb-6">PLATFORM COVERAGE COMPARISON</h3>
        <div className="space-y-4">
          <div>
            ...
            <div className="relative h-8 bg-black/50 rounded-full overflow-hidden">
              <div 
                ...
                style={{
                  width: `${animatedValues.csoc || 0}%`,
                  background: 'linear-gradient(90deg, #ff0044, #ff00ff)'
                }}
              />
              <div className="absolute inset-0 flex items-center px-4">
                <span className="text-xs text-white/80">50,237 / 262,032 assets</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative h-8 bg-black/50 rounded-full overflow-hidden">
              <div 
                className="h-full rounded-full transition-all duration-1000"
                style={{
                  width: `${animatedValues.splunk || 0}%`,
                  background: 'linear-gradient(90deg, #c084fc, #a855f7)'
                }}
              />
              <div className="absolute inset-0 flex items-center px-4">
                <span className="text-xs text-white/80">167,517 / 262,032 assets</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-2">
              <span className="text-green-400">Chronicle (Direct)</span>
              <span className="font-mono text-green-400">{animatedValues.chronicle?.toFixed(1) || 0}%</span>
            </div>
            <div className="relative h-8 bg-black/50 rounded-full overflow-hidden">
              <div 
                className="h-full rounded-full transition-all duration-1000"
                style={{
                  width: `${animatedValues.chronicle || 0}%`,
                  background: 'linear-gradient(90deg, #00ff88, #00ffff)'
                }}
              />
              <div className="absolute inset-0 flex items-center px-4">
                <span className="text-xs text-white/80">241,691 / 262,032 assets</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="glass-panel rounded-2xl p-6 bg-red-500/5 border border-red-500/30">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 bg-black/50 rounded-lg">
            ...
            <p className="text-xs text-gray-300">Deploy 5 regional collectors. Current 12.3% coverage is critical risk.</p>
          </div>
          <div className="p-4 bg-black/50 rounded-lg">
            <div className="text-sm font-bold text-yellow-400 mb-2">PRIORITY 2: LINUX SYSTEMS</div>
            <p className="text-xs text-gray-300">Configure rsyslog on 24,001 Linux servers (30.71% gap).</p>
          </div>
          <div className="p-4 bg-black/50 rounded-lg">
            <div className="text-sm font-bold text-yellow-400 mb-2">PRIORITY 3: DLP EXPANSION</div>
            <p className="text-xs text-gray-300">Deploy DLP to 97,465 unprotected assets immediately.</p>
          </div>
        </div>
      </div>
    </div>
